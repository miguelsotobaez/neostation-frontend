 ...
 *
 * A CHD stores every CD frame as a full 2352-byte sector whatever the track's
 * declared size, so the payload always sits behind a header: 16 bytes for mode
 * 1, and 24 for mode 2, whose 8-byte subheader follows. The sector's own sync
 * pattern is the reliable way to tell which — the track type describes the
 * source image, and a sector whose sync was stripped at compression time has it
 * regenerated by the time it is read back. */
static int32_t nchd_sector_data_offset(const uint8_t *sector, int32_t mode) {
  if (memcmp(sector, kSyncHeader, sizeof(kSyncHeader)) == 0) {
    /* Byte 15 is the sector mode. */
    return sector[15] == 2 ? 24 : 16;
  }
  return mode == NCHD_MODE_MODE2 ? 24 : 16;
}
